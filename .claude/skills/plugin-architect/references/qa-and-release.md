...
## 5. After the release
- Verify the public marketplace repo (`BillyRybka/authentic-ai`, remote `public`) received the push. That repo is the distribution channel. Clients pull the new version on their next marketplace refresh: automatic if they enabled auto-update for `authentic-ai` (off by default for third-party marketplaces), otherwise `/plugin marketplace update authentic-ai`. Nothing else needs publishing, but a client who never refreshes stays on the old version silently.
