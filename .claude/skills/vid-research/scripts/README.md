...
Pulls a channel's videos (last N days, excluding shorts and live streams) via the YouTube Data API v3. Returns JSON with channel metadata, computed median view count, 2x outlier threshold, and per-video data (title, view count, thumbnail URL, duration, published date).
...
3. Skill identifies confirmed outliers (after 2x rule + raw-count threshold + fluke filter).
