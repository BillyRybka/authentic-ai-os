---
name: vid-capture
description: Capture or create a story, metaphor, proof, or testimonial and save it to the creator's evergreen banks. Runnable standalone anytime raw material lands (a client win, a DM, a metaphor mid-conversation, a screenshot), OR invoked by another vid- skill mid-script when the banks don't have what the script needs. Handles both logging material the creator already has and walking them through building something from scratch. Triggers on "capture a story", "new story for the bank", "add to story bank", "save this metaphor", "metaphor capture", "help me come up with a metaphor", "brainstorm a metaphor", "I need a metaphor for", "proof bank", "save a client win", "log a testimonial", "new testimonial", "just got a screenshot", "add to my evergreen banks", or when any other vid- skill asks "do you have a story/metaphor/proof for X".
---

# Video Capture

Capture one item at a time (stories, metaphors, proof, or testimonials) into the creator's banks. This skill is how raw experience becomes usable material at script-writing time. Without these banks, every future script starts empty and fabrication pressure goes up.
...
The four stage flows (S, M, P, T) are identical in both modes. The difference is only in the router and the session-close behavior.
...
- Right after a client wins (capture the story + the proof)
...
- A downstream writing skill (vid-segment, vid-hook) asked for a story that doesn't exist yet
...
3. Ask the creator what they're capturing: story, metaphor, proof, or testimonial.
...
3. **Dig deeper.** Do NOT accept the first pass as final. Probe for specifics:
   ...
   - Typically 2 to 3 rounds. Loop until specificity emerges, then save. Flag in Notes if still thin.
...
7. **Dedup check.** Scan `banks/story-bank/*.md` for matches on `problem_illustrated` + theme tags + slug proximity + first-sentence overlap of the Problem section. If candidates found, show them and ask: update existing, save as new angle, or merge manually.
...
- **Log path.** Creator already has a metaphor (invented it, heard it from someone else, remembered an old one). Validate it through steps 1 to 3, then capture.
- **Create path.** Creator has a concept but no metaphor. Walk them through steps 1 to 3 to generate one. Push back when attempts feel abstract or forced.
...
4. **Classify visual vs. non-visual.** Ask: "Does this metaphor depend on a prop or graphic to land, or does pure speech carry it?" Set `visual: true` or `visual: false`. Visual metaphors capture TWO body sections (Spoken + Shown); non-visual capture one (The metaphor).
...
8. **Assemble the entry** using `assets/metaphor-entry-template.md`. Body depends on `visual`: if true, include Spoken + Shown subsections; if false, single "The metaphor" block.
9. **Read-aloud test.** The metaphor has to land fast. For non-visual, read it aloud with no visual aid — does the viewer still get it? For visual, imagine Spoken + Shown together — does the spoken layer drag without the visual, or does the visual land without the speech?
...
9. **Dedup check.** Scan `banks/proof-bank/*.md` for matches on `proof_type:` + `client:` + first-sentence overlap of "What it proves". If candidates found, show them and ask: update existing, save as new angle, or merge manually.
...
8. **Dedup check.** Scan `banks/testimonial-bank/*.md` for matches on `client:` + `source:` + first-line of verbatim quote. If candidates found, show them and ask: update existing, save as new angle, or merge manually.
...
- **`used_in: []` starts empty.** Writing skills (vid-segment, vid-hook) update it later when they actually use the entry. Never touch it from here after initial creation.
- **Client mention, People stub.** No exceptions. Orphan wikilinks break the graph.
- **Tags per type:** Story gets `story`, `problem-{n}`, optional theme slug. Metaphor gets `metaphor`, `category-{slug}`, `problem-{n}`. Proof gets `proof`, `{proof-type-slug}`. Testimonial gets `testimonial`, `source-{slug}`.
...
- **`creator-foundation.md` frontmatter malformed** (Top 3 problems section unreadable): show the creator what was found vs. expected. Ask: "Want to skip `problem_illustrated` this session (default to 'general'), or pause and fix creator-foundation first?"
...
- People stubs created (names + bucket)
...
References live in `knowledge/` (loaded by multiple skills — vid-segment and vid-hook will load them too):
