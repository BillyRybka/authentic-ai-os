...
Suggested fix: Verify the number with the source (your records on Steve), capture a proof entry via vid-capture, then re-include. OR rewrite as "one of my coaching clients went from zero to a strong monthly recurring number" until proof lands.
```
