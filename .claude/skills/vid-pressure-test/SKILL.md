...
Soft requirements:
- `foundation/reference-pieces/*.md` (rhythm anchor for voice-authenticity reviewer)
...
2. `Content/pieces/{slug}/piece.md` (goal, format, viewer_stage drive rubric weighting)
3. `Content/pieces/{slug}/brain-dump.md` (traceability source)
4. `foundation/creator-foundation.md` (avatar, Top 3, credibility brags)
5. `foundation/voice-profile.md` (words_avoided, anti-patterns, preferred hooks, rhythm signature)
6. `foundation/reference-pieces/*.md` (rhythm exemplars)
...
- foundation docs missing → reject. "Run vid-foundation sequence first."
...
Reads against voice-profile.md (words_avoided, anti-patterns, opener pattern, energy baseline), brand.md (banned words, required swaps), and reference-pieces (rhythm signature). Returns top 3 lines that fail the read-aloud test with quote + suggested rewrite in the creator's voice.
...
- **Hard issue:** factual break (fabricated claim), banned-phrase violation, retention-killer (early payoff, orphaned promise), voice violation against words_avoided
...
- Any words_avoided from voice-profile → surface inline: "'{word}' is on your words_avoided list. Revise or accept?"
...
4. Foundation docs missing → redirect to vid-foundation
...
- `vid-foundation` (sequence) produced creator-foundation, voice-profile, packaging-system
