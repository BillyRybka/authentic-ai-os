...
- `foundation/reference-pieces/*.md` for rhythm reference
...
3. **Confirm viewer_stage.** Default to the temperature predicted in Phase 3. If goal/temperature mismatch (e.g., sales goal + cold temperature), surface the conversion math, see `references/audience-temperature-fit.md`, and let the creator decide.
4. **Capture dropped angles' rationales.** One line each. Sticky for future runs on this piece.
