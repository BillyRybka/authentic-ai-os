...
- `foundation/voice-profile.md` exists (writing skills depend on it for Layer 1 rules and Layer 2 context maps)
...
- `foundation/reference-pieces/*.md` (live rhythm exemplars)
...
3. `foundation/voice-profile.md` (Layer 1 core, Layer 2 context maps, anti-patterns, words avoided)
4. `foundation/reference-pieces/*.md` (live rhythm exemplars matching the format if any)
5. `knowledge/format-planners/{format}.md` (matched to `piece.md` `format:` field)
6. `knowledge/voice-rhythm.md` (sentence-length variation, paragraph ratio, opener pattern, punctuation signature, energy modulation)
...
2. **Reference pieces.** If `foundation/reference-pieces/{slug}.md` files exist that match the format (long-form, listicle, case-study), read them as live rhythm exemplars. Match the cadence pattern (sentence-length variation, paragraph ratio, opener pattern from `voice-rhythm.md`).
3. **Voice profile rules.** Layer 1 anti-patterns are hard rejects, words-avoided are soft rejects with auto-swap, recurring phrases should appear once or twice in long-form segments.

**Per-block writing.**

- **Setup prose.** Pull verbatim from brain dump where possible. If the creator wrote "I was staring at my inbox at 2am wondering when this stopped being fun," that line goes in. Match opener pattern from voice-profile (declaration / question / anecdote / contrarian). Setup runs short: 1-3 sentences for tight formats (news, short process), 3-6 for listicle/deep-dive.
...
- **Payoff prose.** One clear sentence of takeaway. The "what they walk away with." Match voice-profile's punch-out pattern (often a short sentence after a longer one, per `voice-rhythm.md` short-short-long, snap back).
...
**Voice pressure-test inline.** Run the checks from `knowledge/voice-pressure-test.md` Pass 1 silently as you write:

- Anti-patterns from voice-profile present? Hard reject; rewrite.
- Words-avoided present? Soft reject; auto-swap to preferred replacement.
- POV consistency holding? Match the voice-profile defaults.
- Recurring phrases echo at least once in long-form? If zero, voice signal is weak. Fold one in if the brain dump supports it.
...
**Voice pressure-test full pass.** Run the Pass 2 (Layer 2 context map) checks from `knowledge/voice-pressure-test.md` against the locked prose. Sentence rhythm distribution, paragraph structure ratio, punctuation signature, opener pattern, energy match, all against the matched context map (long-form / shorts / linkedin / etc.) for the format. Log result tier (pass / soft-warn / soft-reject / hard-reject). Hard-reject means the prose violates Layer 1 anti-patterns or POV. Restructure, do not save.
...
voice_pressure_test:
  date: YYYY-MM-DD
  result: pass | soft-warn
  layer1_pass: true
  layer2_context: youtube-script
  layer2_pass: true
  flags: []
...
| `foundation/voice-profile.md` | Layer 1 + Layer 2 voice contracts |
| `foundation/reference-pieces/*.md` | Live rhythm exemplars (loaded for piece-level cadence) |
| `Content/pieces/{slug}/piece.md` | Format, goal, pillar, prior banks pulled |
