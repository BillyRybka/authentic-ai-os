# Segment Scaffold (Phase 2 structure draft)

Used by vid-segment Phase 2 to surface a structure draft to the creator. Slot-fill from the segment's bank pulls and format planner. Surface to creator BEFORE writing prose.
...
Proof candidates:
- {proof-slug-1}: {presentation format: static-screenshot | before-after-pairing | live-clip | inline-stat}
- {proof-slug-2}: {presentation format}
...
If this is the FINAL body segment, the transition is body-to-ending. Defer to vid-ending for the bridge construction.

---

## Structure check questions for the creator
...
If structure pass fails 3+ rounds without lock, the segment's job in piece.md is probably wrong. Stop and route back to vid-framing.
