...
**Scope boundary:** this skill produces THE title only. It does not write thumbnails (that's `vid-thumbnail`), hooks (`vid-hook`), or scripts. If the creator wants to also generate thumbnail text, they run `vid-thumbnail` separately or via the orchestrator.

## What this produces

A locked title for one video, saved to `Content/pieces/{slug}/meta.md` (the `title:` field). When invoked as a sub-skill by `vid-structure` or `vid-pipeline`, returns the title string to the caller instead.
...
Hard requirements:
- `foundation/creator-foundation.md` exists with avatar + Top 3 problems (so candidates align with what the audience cares about)
- `foundation/packaging-system.md` exists with the committed BENS strategy and format
- `Content/pieces/{slug}/` exists with at minimum `meta.md` OR a brain-dump / framing artifact that explains what the video is about

If the foundation docs are missing, hard stop — tell the creator to run `vid-foundation` first.

If `banks/title-bank.md` is missing, fall back to using BENS-framework patterns directly + the title-bank seed at `${CLAUDE_PLUGIN_ROOT}/skills/vid-foundation/assets/title-bank-seed.md` (or in the current dev workspace, `assets/title-bank-seed.md`). Note in the brief: "Title bank not yet scaffolded — using seed patterns only."

## Invocation modes

**Standalone:** creator invokes directly. After lock, save the title to `Content/pieces/{slug}/meta.md` and end.

**Sub-skill:** another skill (vid-structure, vid-pipeline) invokes mid-pipeline. Skip the save step; return the locked title string to the caller. The caller writes it to meta.md as part of its own flow.
...
2. `foundation/packaging-system.md` — format, committed BENS strategy
...
5. `Content/pieces/{slug}/meta.md` — the video's format, goal, pillar
6. `Content/pieces/{slug}/brain-dump.md` AND/OR `reference-block.md` AND/OR `script.md` — whatever exists. Pull the actual angle, the specific numbers, named methods, story moments.
...
- Case Study → S (specific receipts) + B (transformation size)
- Short Process → E (achievable, defined steps) + B
- Roast → N (contrarian) + B
- Deep Dive → B + N + S (authority through depth)
- Interview → S (borrowed credibility) + N (unexpected take)
- News → N (timely) + B (stakes)
- Listicle → E (numbered, digestible) + N
...
Each candidate must:
- Be ≤ 50 characters (hard ceiling — YouTube cuts off after 50)
...
- "I Made $12M Selling A Fruit" — subject + verb + specific
- "Why You Should Eat Chocolate Every Day" — conversational, complete
- "The 21 Principles of the Top 0.01%" — defined number + specific group, one phrase
- "Top 10 Most HARMFUL Foods People Keep EATING" — superlative + specific group, flows

Examples that FAIL (and would be rejected):
- "365 To 405 In 11 Weeks (3 Changes)" — three fragments + a clutter parenthetical. No human says this.
...
- "The 12-SOP Rule" — "12-SOP" is a count, not a duration. Compare "The 90 Minute Rule" — "90 minute" describes time, which reads as a system. "12-SOP" reads as a label/stat.
...
**Hard filters — these are genuine constraints, auto-reject:**
...
- **"How I [verb] [specific thing] [in/after/before/etc] [context]"** → "How I Added 50 Pounds To My Squat In 12 Weeks"
- **"Why [subject] [verb-phrase]"** → "Why Looking Poor Is Important"
- **"I [past-tense action] [object/number] [tag]"** → "I Made $12M Selling A Fruit" / "I QUIT My $120,000 Job After Learning 3 Things"
- **"The [number/superlative] [thing] That [verb-phrase]"** → "The 21 Principles of the Top 0.01%"
- **"[Number] [adjective] [things] [target] [verb-phrase]"** → "Top 10 Most HARMFUL Foods People Keep EATING"
- **"Stop [-ing verb] [object]"** → "Stop Asking This!" (single command, not two)
- **"What [subject] [verb] About [topic]"** → "What [Group] Won't Tell You About X"
...
**3. Specificity = credibility.** "$14,332" beats "make money." "23:07 → 19:42" beats "I got faster." "400 sq ft" beats "small apartment." When pulling from the script, default to the most specific number available. Round numbers feel made up; cents and decimals feel real.
...
**5. Stakes / urgency drive clicks.** Why should the viewer act now or pay attention? "(Avoid This Mistake)" / "Before It's Too Late" / "Stop Doing This" — when used honestly (not clickbait), urgency tells viewers their inaction has a cost.
...
> "Which one? Or want me to regenerate with a different angle / hit different BENS letters?"

Wait. If they pick → Phase 3.

If they want changes:
- "Different angle" → re-generate using a different problem-hook from the Top 3
- "Different BENS" → regenerate weighted toward the letters they want (e.g. "more N less B")
- "Shorter" → regenerate under 40 chars
- "Make it more specific" → pull more script-verbatim numbers / named methods into candidates
- "Scrap and start over" → re-run Phase 1, often with the creator pasting new framing
...
**If standalone mode:**
- Save the title to `Content/pieces/{slug}/meta.md` `title:` field
- Update `meta.md` `last_refreshed:` to today's date
- Confirm save: "Title locked: '{title}'. Saved to meta.md."
...
| `foundation/packaging-system.md` | Format, committed BENS strategy |
...
- `vid-thumbnail` — the title and thumbnail will pair; coordinate to avoid word repeats
- `vid-structure` (future) — invokes this skill during STRUCTURE phase
- `vid-pipeline` (future) — orchestrator that calls this skill via vid-structure
- `vid-measurement` (future) — post-publish analysis logs winning titles back into `banks/title-bank.md` and `banks/packaging-bank/`
