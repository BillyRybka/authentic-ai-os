...
The picked idea hands `vid-braindump` a seed packet: `{idea_title, pillar, top_3_problem, iceberg_fit, anchor}`. The anchor MUST carry the full receipt: the source title, the @channel, the views, and the xMed, exactly as they appear in the pattern-bank row. The receipt is the seed's evidence chain: `vid-braindump` persists it into piece.md and `vid-title` inherits it from there as the leading candidate to beat. A seed packet without the full receipt is a broken handoff.
