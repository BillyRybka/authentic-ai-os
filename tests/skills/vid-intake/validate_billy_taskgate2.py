...
# 1) no em-dash / en-dash and no double-hyphen-as-dash (vale_rules scanners)
...
ok_pc, missing_pc = has_fields(pc_fm, pc_required)
if not ok_bd:
    failures.append(f"brain-dump frontmatter missing/empty: {missing_bd}")
if not ok_pc:
    failures.append(f"piece frontmatter missing/empty: {missing_pc}")

# 3) every cited [[bank/...]] link inside the Material section resolves to a
#    file under fixtures/billy/banks. people/ links are allowed and need NOT exist.
...
print("BRAIN_FM_OK:", ok_bd, "missing:", missing_bd)
print("PIECE_FM_OK:", ok_pc, "missing:", missing_pc)
