---
name: aaios-feedback
description: The Authentic AI OS feedback channel. Capture a creator's feedback on an Authentic AI OS skill that just ran (or the session as a whole) and send it straight to Billy, the plugin's author. This is AAIOS-specific, not a generic feedback command, so a creator always knows the report goes to the plugin author and not to Claude or anyone else. Reconstructs which skill ran and where it went sideways, builds a reproduction bundle (a replay case, a snapshot of the determinative vault files, and the bad output) so Billy can recreate the problem, asks one or two light questions, previews everything for consent, then submits to the aaios-feedback form in peak-tools via a public Convex endpoint. Good feedback and rough feedback both welcome. Use this skill whenever a creator wants to report on the Authentic AI OS plugin or one of its skills. Phrases like "give feedback", "leave feedback", "send feedback", "AAIOS feedback", "that didn't work", "report a bug", "this is broken", "something went wrong", "that's not what I wanted", "this skill sucked", or any moment a creator is plainly telling you an Authentic AI OS skill missed. Also invoked by the offer protocol in knowledge/feedback-offer.md when a skill fails, a creator is clearly frustrated, or a full journey completes.
---
