...
**Anti-pattern: no threads.** A script with five disconnected segments (each a complete lesson) reads as flat. The viewer learns each thing but feels no pull. Add at least one thread.
...
**Stacked lessons (no threads).** Five segments, five disconnected lessons, no thread connecting them. Reads as flat. The viewer learns but doesn't binge. Add at least one thread spanning 3+ segments.
...
2. **Locate the title-promise payoff.** Which segment delivers the named answer? Push it late.
3. **Plan threads.** Pick 1-2 threads to run across the body. Mark which segment opens each, which closes it.
...
The tension plan gets recorded in piece.md (`tension_plan:` block) and in script.md as inline notes per segment.
...
- Early-payoff check: is the title's promise resolved before segment 60%?
