...
The universal 6-part architecture every video intro draws from. Loaded by `vid-hook` and any other writing skill that needs to produce or evaluate an intro. Format planners (`knowledge/format-planners/{format}.md`) specify HOW each format adapts this template — which parts apply, which trim out, which expand.
...
Before writing the intro, identify the top 3 questions a viewer would have answered in this video, derived from the locked title + thumbnail combination.
...
**Quick method:** drop the thumbnail + title into a generative-AI prompt and ask "what are the top three questions someone would want answered if they click this?" Use the answers as a draft, then refine.

**Example (thumbnail-text "MAKE BORING THUMBNAILS" + a related title):**
1. What's the trick?
...
The pivot phrase is "The thing is..." — this is what introduces the cost of the problem.
...
When to use: when both sides matter — the problem is high-emotion AND the result is impressive. Most powerful option, but longest.
...
Where Q1-Q3 are the Top 3 viewer questions from Step 1. Maximum 3. If the format suggests fewer, fewer is fine. More than 3 = overwhelming.
...
| Studio + face | Same studio + face in the open |
...
This is a constraint the writing skills surface but the creator/editor enforces.
...
| Short Process | Compressed — Hook + tight promise + transition to step 1. Setup is one line. |
| Listicle | Hook + count tease + transition ("Starting with #1..."). Credibility woven into the count. |
| Roast | Hook is the contestant submission + 1-line "what we're roasting." Setup compressed. |
| News | Hook + 1-line context + immediate transition. NO Setup, NO Credibility line — speed matters. |
| Interview | Hook is the GUEST's credibility + "today we're talking about X" + question setup. Different shape entirely. |

## How vid-hook uses this file

When `vid-hook` runs, it should:
...
These are patterns that tend to under-perform. The skill should flag them and explain why — not auto-reject. The creator can override if they have reason to.
...
**Soft friction (often suboptimal, but creator may have reason):**
- Credibility Hook on small/new channels — usually fails the cold-trust test, but a single dramatic result can earn it
- Setup with more than 3 things — usually overwhelms, but a complex methodology may need it
- Hook longer than 5 seconds — usually wastes attention, but a story-driven open can run longer if it earns the time
- Whole intro over 30 seconds — usually means teaching crept in, but some formats (Deep Dive) earn longer setup
- Teaching content inside the intro — usually a hook-killer, but contextual setup that LOOKS like teaching can land
- Problem-poking that doesn't match one of the avatar's Top 3 problems — usually misses the audience, but a fresh angle on a related problem can work
