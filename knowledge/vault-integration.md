...
Frameworks are the creator's OWN named systems/structures/mental models. The teachable patterns they repeat across videos. Third-party frameworks do NOT go here. They live in `knowledge/` or `resources/references/` with attribution.
...
The audience-family schemas (audience-data, vocabulary-sample, audience-segment, avatar, held-out, avatar-validation, avatar-review, panel-synthesis) are parked with the WIP aud-* skills at `.claude/skills-wip/vault-integration-aud-schemas.md`.
