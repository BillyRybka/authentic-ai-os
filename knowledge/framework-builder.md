...
type: reference
scope: shared
loaded_by: [vid-capture, vid-segment]
status: active
...
This is the spine of inline framework crafting in vid-segment. Walk the creator through these steps. They produce a named system the creator can either lock into the script directly, or save via vid-capture Stage F for reuse across future videos.
...
When the creator wants to save a framework (either standalone via vid-capture Stage F or after inline crafting in vid-segment), the entry follows this shape. Stage F handles the actual save.
