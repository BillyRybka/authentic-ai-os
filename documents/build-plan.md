...
Packaging runs before structure. A weak package is where the creator stops, before any time goes into scripting.

When `vid-pipeline` runs the PACKAGING phase (after framing, before structure):

1. Reads `brain-dump.md`, `piece.md` (angle + format locked by `vid-framing`)
2. INVOKES `vid-title` → returns the locked title
3. INVOKES `vid-thumbnail` → writes `thumbnail-brief.md`
4. No script work happens until title + thumbnail are locked.

When `vid-pipeline` runs the STRUCTURE phase (packaging already locked):

1. Reads `brain-dump.md`, `piece.md` (including the locked title + thumbnail)
2. Extracts distinct ideas (lightweight logic in the orchestrator)
3. Runs keep/cut/combine review with the creator
4. Surfaces block candidates per segment and writes the `## Blocks to capture` manifest to `script.md`
5. Assembles the Tier 1 skeleton (the outline) and saves to `script.md`

When the SCRIPT phase runs, the creator first fills any open blocks (batch now, or inline as they write), then `vid-intro` writes the hook, `vid-segment` writes each body section, `vid-ending` writes the close. Title and thumbnail are never re-opened here.
