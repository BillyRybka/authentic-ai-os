...
- **READS**: `creator-foundation.md`, `voice-profile.md` (alignment checks), `knowledge/story-capture-guide.md`, `knowledge/framework-builder.md`, `knowledge/vault-integration.md`, plus skill-local `references/metaphor-builder.md`, `references/proof-capture-guide.md`, `references/testimonial-capture.md`
...
### vid-framing `STAGED`
Picks the single angle for one video, built on one ordering principle: psychology first, evidence second. It reads the brain-dump, then gets into the one viewer's head (what they want, the main problem they are stuck on, the tension underneath, the transformation, the core payoff) and lays that read back in plain lines, waiting for a "that's the video" before building anything, because a thin read confirms politely and costs the whole video downstream. Only after that yes does it hunt the reframe: one of five moves from `references/reframe-toolkit.md` (a fresh comparison, a contrarian flip, a named system, the creator's own story, a visual framework), each judged against BENS with the N weighted hardest, then grounded by the anchor rule out of `references/angle-anchor-rules.md` (name the real bank entry with its title, channel, and views, or call it a gut pick; nothing in between). The creator picks the angle, names the goal, and locks the format the frame implies (sanity-checked against the goal in `references/format-index.md`). Every dropped angle gets a one-line reason and stays sticky across re-frames, and it never invents an outlier, number, or result; a withheld proof point becomes a one-line TODO.
- **READS**: `brain-dump.md`, `piece.md`, `creator-foundation.md`, `foundation/voice-profile.md` (the refusals list), `banks/pattern-bank.md`, `knowledge/BENS-framework.md`, `knowledge/vault-integration.md` (the field-ownership map at save), plus skill-local `references/reframe-toolkit.md`, `references/angle-anchor-rules.md`, `references/format-index.md`
- **WRITES**: `piece.md` (`selected_angle`, `core_payoff`, `format`, `goal`, `voice_context`) + the dropped-angles body section
...
- **READS**: `piece.md`, `brain-dump.md` (plus `script.md` if it exists), `creator-foundation.md`, `banks/pattern-bank.md`, `banks/title-bank.md`, `banks/power-words-bank.md`, `knowledge/BENS-framework.md`, `references/title-filters.md` (on demand: soft flags, natural English shapes, the no-banks fallback)
...
| `knowledge/vault-integration.md` | 16 skills | The routing, frontmatter, and wikilink rules contract |
...
- **vid-title points into vid-framing's `references/`.** The anchor rule's canonical statement lives at `references/angle-anchor-rules.md` in the vid-framing folder. Fine in the repo; when both skills ship, the pointer needs to resolve plugin-relative.
...
- **The aud-* method and schemas are parked, not lost.** `synthetic-audience-method.md` and `vault-integration-aud-schemas.md` sit at `.claude/skills-wip/` next to their only consumers, and `knowledge/vault-integration.md` is slimmed to 618 lines with a pointer to the parked schemas.
