...
### vid-intake `STAGED`
Captures one video's raw material into `brain-dump.md` in the creator's exact words. It does not frame, title, or write. Five steps on one spine: take the dump (read once, respond once, and put `piece.md` on disk the moment the topic is known), mirror it back and checkpoint `brain-dump.md` before digging so a dropped session cannot lose their words, dig until the material stops opening (one question at a time, no permission round, verifying uncertain claims via an isolated subagent, turning gaps into TODOs not inventions), confirm iceberg fit and name the likely pillar, then hand to vid-framing. It carries an internal taxonomy of material shapes (story, client-win, unbacked claim) it handles without ever telling the creator. The dump leads with a lossless verbatim `## Raw dump` as the source of truth, with organized sections layered on top as an index, and every save passes a Vale check for em-dashes. Cold open: it loads nothing until a step needs it, the fit check is iceberg-only (no Top 3), and it never loads voice.
- **READS**: `foundation/iceberg.md` (the fit check, not cold), `knowledge/story-capture-guide.md` (only when a thin story needs drilling), plus skill-local `references/digging-deeper.md` (at the dig) and `references/verify-subagent.md` (on an uncertain claim)
...
### vid-framing `STAGED`
Reads the one viewer and locks what the video argues. It owns the argument and never the words: no banks, no BENS, no headline-shaped output, because packaging with the lock list and the kill pass is vid-title's job and running it twice only anchors the title session on a line written blind. It reads the brain-dump, then writes the read as four fields in third person (Target, Transformation, Stakes, Core payoff, the shape borrowed from the source method's audience-targeting framework), where Target carries the blind spot only when the material has one and Stakes compound three to five consequences deep. It lays that back one field per paragraph and waits for a "that's the video" before building anything, because a thin read confirms politely and costs the whole video downstream. Then it names the argument that closes the gap between where the viewer is and where they land, digging via `references/stake-finder.md` when Target or Stakes came up flat, and naming the ceiling out loud rather than manufacturing tension when two rounds surface nothing. The creator picks the argument, names the goal, and the format the frame implies gets locked against `references/format-index.md`. Dropped angles keep a one-line reason and stay sticky across re-frames, and it never invents a number or result; a withheld proof point becomes a one-line TODO. The dig runs one question at a time, never a battery (what do people get wrong, what did you believe before, what does it cost them, who pushed back), and the four blind-spot shapes in `stake-finder.md` are the shapes it listens for. It does not kill videos; measurement does. `## The Read` is contracted in `knowledge/piece-contract.md` and read downstream: vid-title presses on the Stakes, vid-intro mines them for hooks, vid-structure builds toward the Transformation, all three degrading to prior behavior when the section is absent.
...
- **WRITES**: `piece.md` (`frame`, `core_payoff`, `format`, `goal`, `voice_context`) + the `## The Read` body section (Target, Transformation, Stakes, Core payoff) and the dropped-angles body section
...
- **READS**: `piece.md` (including framing's `## The Read`; the Stakes are what the title presses on), `brain-dump.md` (plus `script.md` if it exists), `creator-foundation.md`, `banks/pattern-bank.md`, `banks/title-bank.md`, `banks/power-words-bank.md`, `knowledge/BENS-framework.md`, plus on demand `references/title-filters.md` (soft flags, natural English shapes, the no-banks fallback), `references/angle-anchor-rules.md` (the canonical receipt rule), `references/reframe-toolkit.md` (the five shapes for a stake that is not landing plainly)
